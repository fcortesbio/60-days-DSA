    ...
    for function in functions:
        try:
            timing = timeit.timeit(
                lambda f=function: f(test_array), number=replicas, timer=timer
            )
            result = BenchmarkResult(
                function=function, time=timing, iterations=replicas
            )
            ...
            results.append(
                BenchmarkResult(
                    function=function, time=float("inf"), iterations=replicas
                )
            )
...
def analyze_performance(
    ...
    if len(valid_results) > 1:
        ...
        if fast_count > 0:
            print(
                f"   • Fast algorithms (< {format_time(MEDIUM_THRESHOLD)}): {fast_count}"
            )
        if slow_count > 0:
            print(
                f"   • Slow algorithms (> {format_time(SLOW_THRESHOLD)}): {slow_count}"
            )
...
def _analyze_algorithm_categories(
    results: list[BenchmarkResult], complexity_map: dict[str, str]
) -> None:
    """
    ...
    if brute_force_algos:
        ...
        if linear_algos and brute_force_algos:
            ...
            print(
                f"   • Algorithmic improvement: {improvement:.0f}x faster with optimal complexity"
            )
...
def compare_scale_performance(
    ...
    for func_name in sorted(common_functions):
        ...
        if (
            small_result.iterations
            and large_result.iterations
            and small_result.iterations > 0
            and large_result.iterations > 0
        ):
    ...
    print(
        f"   • Theoretical O(n²) should scale quadratically ({(large_size / small_size) ** 2:.0f}x)"
    )
